...
def main():
    ...
    for dir in dirs:
        ...
        for bag_file in bag_files:
            ...
            plt.ylabel('error [m]')
            plt.legend()
            plt.title(f'absolute position error final errors \n odometry filtered: {odom_filtered_error[-1]:.4} odometry: {odom_error[-1]:.4}') #amcl: {amcl_error[-1]:.4}')
            ...
            axs[0].set_title('odometry angle error')
            axs[1].set_title('filtered odometry angle error')
